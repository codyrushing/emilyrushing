...
/*#__PURE__*/
function () {
  function WorkApp(_ref) {
    var root = _ref.root;

    _classCallCheck(this, WorkApp);
  ...
  _createClass(WorkApp, [{
    ...
    value: function init() {
      ...
      var readURLState = function readURLState() {
        var search = window.location.search.replace(/^\?/, '');
        _this.activeFilters = new Set(search.replace(/^\?/, '').split(',').map(decodeURIComponent).filter(function (s) {
          return s;
        }));
    ...
    value: function buildTagNav() {
      ...
      this.workItems.forEach(function (item) {
        ...
        _this2.itemsMap.set(item, itemTags);

        _this2.tags = _this2.tags.concat(itemTags);
      }); // flatten and remove dupes

      this.tags = this.tags.reduce(function (acc, v) {
        if (acc.indexOf(v) === -1) {
          acc.push(v);
        }
      ...
      this.tags.forEach(function (tag) {
        ...
        _this2.tagNav.appendChild(tagLink);
      });
      this.tagLinks = this.root.querySelectorAll('a[data-tag]');
      this.tagLinks.forEach(function (tagLink) {
        var tag = tagLink.getAttribute('data-tag');
        tagLink.addEventListener('click', function (e) {
          e.preventDefault();

          _this2.toggleFilter(tag);
        });
      });
    }
  }, {
    key: "toggleFilter",
    value: function toggleFilter(tag) {
      ...
      this.activeFilters.forEach(function (filter) {
        return newHash.push(filter);
      });
      window.history.replaceState(null, null, "?".concat(newHash.join(',')));
      this.render();
    }
  }, {
    key: "render",
    value: function render() {
      ...
      this.tagLinks.forEach(function (tagLink) {
        ...
        if (_this3.activeFilters.size) {
          _this3.activeFilters.has(tagLink.getAttribute('data-tag')) ? tagLink.classList.add(activeClass) : tagLink.classList.remove(activeClass);
        } else {
          tagLink.classList.add(activeClass);
        }
      });
      this.workItems.forEach(function (item) {
        item.style.display = _this3.activeFilters.size ? _this3.itemsMap.get(item).some(function (tag) {
          return _this3.activeFilters.has(tag);
        }) ? null : 'none' : null;
      });
...
function () {
  ...
  _createClass(App, [{
    ...
    value: function ready() {
      ...
      this.stickyHeader();
      document.querySelectorAll('.site-main .work-list').forEach(function (workAppRoot) {
        return new _work.default({
          root: workAppRoot
        });
    ...
    value: function stickyHeader() {
      ...
      var applyHeaderStyles = function applyHeaderStyles() {
        stickyWrapper.style.height = "".concat(header.offsetHeight, "px");
      };

      window.addEventListener('resize', (0, _lodash.default)(applyHeaderStyles, 500));
      applyHeaderStyles();
    }
  }, {
    key: "stylizeHeadings",
    value: function stylizeHeadings() {
      var headings = document.querySelectorAll('.hero h1, .hero h2, .post-card h3');
...
exports.default = App;
},{"lodash.throttle":"../../node_modules/lodash.throttle/index.js","./work":"work.js"}],"index.js":[function(require,module,exports) {
"use strict";
